...
class Client:
    ...
    def __init__(
        ...
        self.disconnect_if_no_response_for = 5
        self.language = lang
        self.model = model
...
class TranscriptionClient(TranscriptionTeeClient):
    ...
    def __init__(
        ...
        recording_filename = f"{Path.home()}/.cache/{model}_output_recording.wav"
        transcription_path = f"{Path.home()}/.cache/{model}_output.srt"
        self.client = Client(
            host,
            port,
            lang,
        ...
        if save_output_recording and not recording_filename.endswith(".wav"):
            raise ValueError(
                f"Please provide a valid `output_recording_filename`: {recording_filename}"
            )
        if not transcription_path.endswith(".srt"):
            raise ValueError(
                f"Please provide a valid `output_transcription_path`: {transcription_path}. The file extension should be `.srt`."
            )
        TranscriptionTeeClient.__init__(
            self,
            [self.client],
            save_output_recording=save_output_recording,
            output_recording_filename=recording_filename,
